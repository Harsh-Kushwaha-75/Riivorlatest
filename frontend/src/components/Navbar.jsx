...
const Navbar = () => {
  ...
  return (
    <div className='flex items-center justify-between py-5 font-medium bg-black text-white px-6'>
      ...
      <div className='flex items-center gap-6'>
        ...
        <div className={`fixed top-0 right-0 bottom-0 overflow-hidden  bg-white transition-all duration-300 z-50 ${visible ? 'w-full' : 'w-0'}`}>
                <div className='flex flex-col text-black h-full'>
                    ...
                    <NavLink onClick={()=>setVisible(false)} className='py-2 pl-6 border-white/20 border' to='/'>HOME</NavLink>
                    <NavLink onClick={()=>setVisible(false)} className='py-2 pl-6 border-white/20 border' to='/collection'>COLLECTION</NavLink>
                    <NavLink onClick={()=>setVisible(false)} className='py-2 pl-6 border-white/20 border' to='/about'>ABOUT</NavLink>
                    <NavLink onClick={()=>setVisible(false)} className='py-2 pl-6 border-white/20 border' to='/contact'>CONTACT</NavLink>
                </div>
        </div>
